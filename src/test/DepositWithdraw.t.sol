...
contract DepositWithdrawTests is EigenLayerTestHelper {
    ...
    function testFrontrunFirstDepositor() public /*uint256 depositAmount*/ {
        //setup addresses
    ...
    function testDepositTokenWithOneWeiFeeOnTransfer(
        ...
        {
            uint256 actualSharesOut = strategyManager.stakerStrategyShares(sender, oneWeiFeeOnTransferTokenStrategy) -
    ...
    function testForkMainnetDepositSteth() public {
        ...
        if (operatorSharesBefore == 0) {
            ...
            assertTrue(
                strategyManager.stakerStrategyList(
                    address(this),
                    strategyManager.stakerStrategyListLength(address(this)) - 1
                ) == stethStrategy,
                "_testDepositToStrategy: stakerStrategyList array updated incorrectly"
            );
        }
        ...
        // to account for this we check approximate rather than strict equivalence here
        {
            uint256 actualSharesOut = strategyManager.stakerStrategyShares(address(this), stethStrategy) -
                operatorSharesBefore;
            ...
            // additional sanity check for deposit not increasing in value
            require(
                stethStrategy.sharesToUnderlying(actualSharesOut) <= amountToDeposit,
                "value cannot have increased"
            );
            // slippage check
            require(
